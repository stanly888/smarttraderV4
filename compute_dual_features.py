...
def compute_single_features(df: pd.DataFrame) -> tuple[np.ndarray, float, float, float, float]:
    ...
    volatility_factor = np.clip((atr_pct + bb_pct_value) * 10, 0.5, 2.0)

    features = np.array([
        vwap_diff.iloc[-1], obv, mfi, atr,
        bb_width.iloc[-1], price_pos.iloc[-1], ma_diff.iloc[-1],
        rsi, price_chg.iloc[-1], vol_chg.iloc[-1],
        atr_ratio, rsi_zone, bb_pct,
        bb_dev, ma_slope.iloc[-1], close.iloc[-1],
        fib_mean_dist
    ])
    ...
    return normalized, atr, bb_width.iloc[-1], fib_mean_dist, volatility_factor
...
def compute_dual_features(symbol="BTC-USDT") -> tuple[np.ndarray, tuple[float, float, float, float]]:
    ...
    dual_features = np.concatenate([features_15m, features_1h, [current_price]])
